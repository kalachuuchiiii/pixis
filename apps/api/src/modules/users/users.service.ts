...
export class UsersService {
  ...
  async updateUser({
    ...
    const usernameCooldown = await withCooldown('14d', myUser.lastUsernameUpdate?.getTime(), () => {
    ...
    const nicknameCooldown = await withCooldown('7m', myUser.lastNicknameUpdate?.getTime(), () => {
    ...
    const { username, nickname , lastNicknameUpdate, lastUsernameUpdate } = result;
    return {
      username,
      nickname,
      lastNicknameUpdate,
  ...
  async toggleIsPrivate (user: AuthPayload) {
  ...
  async deleteAccount (user: AuthPayload) {
